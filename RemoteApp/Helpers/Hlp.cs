...
namespace RemoteApp.Helpers
{
    ...
    public static class Hlp
    {
       // private static NLog.Logger _logger = LogManager.GetCurrentClassLogger();
        ...
        public static bool SaveChanges(ApplicationDbContext db, bool rollback = true)
        {
            try
            {
                db.SaveChanges();
            ...
            //            _logger.Error($@"Error '{x.ErrorMessage}' occurred in {entityType} at {x.PropertyName}");
            //        RollBack(entry, rollback);
            //    }
            //}
            //catch (DbUpdateException ee)
            //{
            //    foreach (var entry in ee.Entries)
            //    {
            //        RollBack(entry, rollback);
        ...
        public static void Notify(User user, object ob, string type)
        {
            var db = new ApplicationDbContext();
            string msg = null;
            //if (type == "Mesures")
            //    msg = "Ajout de nouvelles constantes médicales pour le patient \n" + Patient((MeasurementGroup)ob);
            //else if (type == "Examens de biologie")
            //    msg = "Arrivée de nouveaux résulats biologiques pour le patient \n" + ((Sejour)ob).Patient;
            //else if (type == "Examens d'imagerie")
            //    msg = "Arrivée de nouveaux résulats d'imagerie pour le patient \n" + ((Sejour)ob).Patient;
            //db.Messages.Add(new Message(user.UserID, type, msg));
            SaveChanges(db);
        }

        //private static Patient Patient(MeasurementGroup i)
        //{
        //    var db = new ClinModel();
