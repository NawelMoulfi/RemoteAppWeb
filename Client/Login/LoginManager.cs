...
namespace RemoteAppWeb.Login
{
    public class LoginManager
    {
        ...
        public async Task<UserDto> TryLogin(string username, string password, bool isSuperUser)
        {
            ...
            UserDto user = await _userDataService.Login(username, password);
        ...
        public static ClaimsPrincipal GetClaimsPrincipal(UserDto user, bool isSuperUser)
        {
            if(user!=null) {
                Console.WriteLine($"userLogin : {user.UserLogin}");
                Console.WriteLine($"userId : {user.UserId}");
                var claims = new List<Claim>
            {
                ...
                new Claim(ClaimTypes.Role, user?.RoleRoleName??""),
            };
                // isSuperUser = true;
                if (isSuperUser)
